...
const getCellValue = (item, headerItem, maps) => {
  ...
  if (headerItem.attr)
    value =
      maps != null && maps.length && headerItem.isMapped
        ? maps[headerItem.mapIndex].get(getValue(item, headerItem.attr))
          ? maps[headerItem.mapIndex].get(getValue(item, headerItem.attr))[
              headerItem.mappedAttribute
            ]
          : getValue(item, headerItem.attr)
        : headerItem.isBooleanOp
        ? getValue(item, headerItem.attr)
  ...
  if (headerItem.condition) {
    ...
    } else value = getValue(item, headerItem.falseAttr);
  }
  ...
  if (headerItem.label) value = value + ' ' + headerItem.label;

  if (headerItem.badge && offersLength > 0) {
    value = (
      <>
        {' '}
        {value} {headerItem.badge}{' '}
      </>
    );
  }
...
function Datatable({
  ...
  const [state, setState] = useState({
    data:
      isLazy == true
        ? datatable
        : datatable.slice(rowsPerPage * page, rowsPerPage * (page + 1)),
    page: page,
    rowsPerPage,
    numberOfPages: size
      ? Math.ceil(size / rowsPerPage)
      : Math.ceil(datatable.length / rowsPerPage)
  });
          ...
          <TableContainer component={Paper}>
            ...
            >
              <TableHead className={classes.dataTableHead}>
                <TableRow>
                  {header.map((item, i) => {
                    return <TableCell key={i}>{item.name}</TableCell>;
                  })}
                  {actions.map((actionItem, i) => {
                    return <TableCell key={i}>{actionItem.name}</TableCell>;
                  })}
                  {hasChild && <TableCell />}
                </TableRow>
              </TableHead>
                                ...
                                <TextField
                                  ...
                                  onChange={(event) => {
                                    if (event.target.value != '') {
                                      ...
                                      event.target.value = getCellValue(
                                        item,
                                        headerItem,
                                        maps
                                      );
                            ...
                            <TableCell key={index}>
                              {actionItem.element ? (
                                ...
                                <Button
                                  ...
                                  round
                                  onClick={() =>
                                    actionItem.action(JSON.stringify(item))
                                  }
                                >
                                  {actionItem.name}
                                </Button>
