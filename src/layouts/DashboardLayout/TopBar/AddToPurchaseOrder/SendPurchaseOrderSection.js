...
} from 'src/redux/slices/partSearch';
import {
  createNewMessage,
  getCompanyUsers,
  getContacts,
...
function SendPurchaseOrderSection(props) {
  ...
  const { userConversations, messages, onlineUsers } = useSelector(
    (state) => state.chat
  );
  ...
  useEffect(() => {
    if (orders.length == 0) props.setOpenSendPO(false);
  }, [orders]);

  const handleClick = (itemID) => {
    if (openItemID != itemID) setOpenItemID(itemID);
    else setOpenItemID(0);
  };

  const getCompanyName = (orderItem) => {
    const companiesFilter = companyiesData.filter(
      (c) => c[0] === orderItem.companyId
    );
    return themeDirection === 'ltr'
      ? companiesFilter.map((n) => n[1].name)
      : companiesFilter.map((n) => n[1].nameAr);
  };
  ...
  const sendOrder = async (order, companyId) => {
    ...
    selectedConversation.members
      .filter((x) => x.id != user.subscriber.id)
      .map((member) => {
        let onlineUserIndex = onlineUsers.findIndex(
    ...
    dispatch(
      updateCompaniesOrders(
        newCompanyOrders.filter((x) => x.companyId !== companyId)
      )
    );
        ...
        <>
          <div key={index}>
            ...
            >
              ...
              {openItemID === orderItem.companyId ? (
                <ExpandLess />
              ) : (
                <ExpandMore />
              )}
            ...
            <Collapse
              in={openItemID === orderItem.companyId}
              timeout="auto"
              unmountOnExit
            >
