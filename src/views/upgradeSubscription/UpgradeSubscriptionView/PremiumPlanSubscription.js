...
function PremiumPlanSubscription({
  settings,
  close,
  addUserCheck,
  addBranchCheck
  ...
  const { loginObject, countries, availablePlans, bancks } = useSelector(
    (state) => state.authJwt
  );
  ...
  const planStatus = loginObject.company.subscriptions.filter(
    (e) => e.status != 'F'
  )[0].status;
  ...
  const branchUserNumCost = (itemType, num) => {
    ...
    else return findPlanItemType(itemType).yearlyPrice * num;
  };
  ...
  const subtotalSubscription =
    findPlanItemType('S').yearlyPrice + branchsNumCost + usersNumCost;
  const discount =
    promotion != null
      ? promotion.discountPercentage *
        (settings ? subtotalSetting : subtotalSubscription)
      : 0;
  const subtotal =
    (settings ? subtotalSetting : subtotalSubscription) - discount;
  const vatAmount = subtotal * 0.15;
  const totalAmount = subtotal + vatAmount;

  const handlePromotionSubmit = async ({ code }) => {
    try {
      const { data: promotionValue } = await paymentService.activePromtion(
        code,
        availablePlans[1].id
      );
      if (promotionValue) {
  ...
  const onAttach = async (event) => {
    ...
    if (!['jpeg', 'png', 'jpg', 'pdf'].includes(extension.toLowerCase())) {
      setReceiptFileError(
        t('File must be in these extensions jpeg, png, jpg, pdf')
      );
      setReceipt(null);
    } else {
  ...
  const paymentBodyObject = (salesType, price, items, country) => {
    return {
      ...
      promoDiscount:
        promotion != null
          ? Math.round(promotion.discountPercentage * price)
          : 0,
      vatPercentage: 0.15,
  ...
  const submitPaymentOrder = async () => {
    try {
      let country = countries.find(
        (e) => e.id === loginObject.company.countryId
      );
      let items = [];
      ...
      for (let index in planItems) {
        const item = {
          ...
          quantity: planItems[index].mandatory
            ? 1
            : getQuantity(planItems[index].itemType)
        };
        items.push(item);
      }

      const itemsToSetting = items.filter(
        (item) =>
          (item.itemType == 'U' || item.itemType == 'B') && item.quantity > 0
      );
      ...
      setLoaded(true);
      enqueueSnackbar(
        error.response.data ? t(error.response.data) : error.response.status,
        { variant: 'error' }
      );
    }
  };
  ...
  return (
    ...
    >
      <Box>
          ...
          {!settings && (
            <ListItem className={classes.totalAmountChild}>
              ...
              <Typography variant="body1" className={classes.totalAmountNum}>
                {findPlanItemType('S').yearlyPrice} {t('SAR')}
              </Typography>
            </ListItem>
          )}
          {!settings && (
            <ListItem className={classes.totalAmountChild}>
              ...
              <Typography variant="body1" className={classes.totalAmountNum}>
                {findPlanItemType('A').yearlyPrice} {t('SAR')}
              </Typography>
            </ListItem>
          )}
          {!settings && (
            <ListItem className={classes.totalAmountChild}>
              ...
              <Typography variant="body1" className={classes.totalAmountNum}>
                {findPlanItemType('D').yearlyPrice} {t('SAR')}
              </Typography>
            </ListItem>
          )}
          {/* <ListItem className={classes.totalAmountChild}>
          ...
          <ListItem className={classes.totalAmountChild}>
            <Box className={classes.plusOptionsCont}>
              <Typography
                variant="body3"
                className={
                  themeDirection == 'ltr' ? classes.smallMarg : classes.bigMarg
                }
              >
                {t('Branches number')}
              </Typography>
          ...
          <ListItem className={classes.totalAmountChild}>
            <Box className={classes.plusOptionsCont}>
              <Typography
                variant="body3"
                className={
                  themeDirection == 'ltr' ? classes.MedMarg : classes.smallMarg
                }
              >
                {t('Users number')}
              </Typography>
        ...
        {promotion == null ? (
          <Box
            className={clsx(classes.spaceBetweenElements, classes.activeBtn)}
          >
          ...
          <Typography
            variant="body3"
            className={classes.PromotionDiscountSuccess}
          >
        ...
        <Button
          disabled={
            paymentMethod == 1
              ? !receipt || receiptFileError != null || totalAmount == 0
              : totalAmount == 0
          }
